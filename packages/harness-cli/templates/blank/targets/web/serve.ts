...
main(function* () {
  ...
  server.on("connection", (socket) => {
    // The `ws` socket structurally satisfies binding's `WsServerSocket`, but
    // binding doesn't attach a 'error' handler — an unhandled 'error' on a Node
    // EventEmitter throws, so the host must.
    socket.on("error", () => {});
