...
export interface Bridge<E, C, S> {
   ...
   *  desktop shell opens the system dialog.
   *
   *  Absent in a browser, and not from neglect: a page is handed file BYTES and
   *  never a path, so there is nothing it could answer with. A view offers the
   *  affordance only where the bridge has one. */
