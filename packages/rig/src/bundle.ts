/**
 * Signed-tarball Ability distribution — verify primitives.
 *
 * Abilities are distributed as signed npm tarballs through the canonical
 * channel at {@link CHANNEL_CATALOG_URL}. The `lloyal install` CLI
 * uses the primitives here ({@link verifyBundle}, {@link resolveAbilityEntry})
 * to fetch + signature-verify a tarball against
 * the vendored trust roots, then shells out to `npm install <URL>` so
 * the ability lands in the harness's `node_modules` like any other npm
 * dependency. The harness boots and imports each ability with a plain static
 * `import`; the framework provides no runtime "load ability by name" verb.
 *
 ...
 * code paths.
 *
 * **Channel-canonical resolution.** {@link resolveAbilityEntry} fetches the
 ...
 * before `lloyal install` invokes `npm install <tarball-URL>`, so
 ...
 * could not import them: rig's entry chain-imports the Ability runtime and the
...
export { BundleVerificationError, AbilityNotFoundError };
 ...
 * {@link resolveAbilityEntry} calls (and the internal catalog-verification
...
//
// A boot session may resolve several abilities from the same catalog
 ...
 * published ability uninstallable. The shared package is pinned against frozen
 ...
 * {@link AbilityNotFoundError} if the name is absent or no version matches.
 *
 * Consumers (notably the `lloyal install` CLI) then fetch the
 ...
 */
export function* resolveAbilityEntry(
  name: string,
  opts: { semver?: string } = {},
  ...
  if (!entry) {
    throw new AbilityNotFoundError(
      `Ability "${name}" is not listed in the catalog at ${getCatalogUrl()}.`,
    );
  }
