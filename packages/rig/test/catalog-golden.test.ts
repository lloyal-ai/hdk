/**
 * The drift gate for the channel's signature encoding — rig's half.
 *
 * **Why a frozen fixture rather than a round-trip.** Every other
 * signing/verification test in this repo signs with one copy of
 * `canonicalJson` and verifies with another copy of the same function —
 * `bundle.test.ts` did this most literally, carrying its own mirror of the
 * helper as its oracle. That catches a one-sided edit and nothing else: a
 * change applied to both sides together sails through. The only oracle that
 ...
 * `apps.lloyal.ai/v1/catalog.json` as served, verified against the trust root
 * committed in `protocol.ts`.
 *
 * **How this proves the two implementations agree.** `harness-cli` has the
 * same fixture and asserts the same SHA-256 over the canonical bytes. Neither
 * test references the other, and neither can be satisfied by matching the
 * other's behaviour — both must match what the *signer* produced. Agreement
 * between the copies is a consequence, not an assumption.
 *
...
describe('frozen production catalog', () => {
  ...
  it('ships the trust root the catalog was signed with', () => {
    // Read straight from `protocol.ts`, NOT through `getTrustRoots()` — the
    // test-env override machinery must not be able to satisfy this.
