/**
 * Scenario: concurrent extendSpine calls are serialized through the tick loop.
 *
 * Before the fix, `PoolContext.extendSpine` issued `store.prefill` directly
 ...
 * extendSpine when their agent completed. Two fibers could hit
 ...
 *
 *   - Multiple concurrent extendSpine calls never produce overlapping
 *     native calls (I1 invariant).
 *   - Each extendSpine completes (emits a `spine:extend` trace event).
 *   - Extend tokens land on the spine branch (spine.position advances).
 */
...
describe('scenario: concurrent extendSpine has no native-call overlap', () => {
  ...
  it('multiple concurrent extendSpine calls do not overlap at the native layer', async () => {
    // Custom orchestrator: spawn 3 agents concurrently, wait for all,
    // then fire 3 extendSpines concurrently via `all(...)`. Without the
    ...
    const flatOrchestrator = function* (ctx: PoolContext): Operation<void> {
      ...
      // The critical bit: three concurrent extendSpine calls. Before the
      // fix, each would independently hit store.prefill from its own fiber.
      yield* all([
        ctx.extendSpine('Task 0', 'Findings 0'),
        ctx.extendSpine('Task 1', 'Findings 1'),
        ctx.extendSpine('Task 2', 'Findings 2'),
      ]);
    };
  ...
  it('chain-style serial extendSpine still works (no regression)', async () => {
    const chainOrchestrator = function* (ctx: PoolContext): Operation<void> {
      for (let i = 0; i < 2; i++) {
        ...
        yield* ctx.extendSpine(`Task ${i}`, `Findings ${i}`);
      }
    };
  ...
  it('tick loop does not exit before final extend drains', async () => {
    ...
    const lateExtendOrchestrator = function* (ctx: PoolContext): Operation<void> {
      ...
      yield* ctx.extendSpine('Task', 'Findings');
    };
